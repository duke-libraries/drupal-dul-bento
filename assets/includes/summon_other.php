...
if($queryTerms != "") {
	...
	echo '<div class="results-block" id="results-other">';
		...
		echo '<h2>Other Resources <a href="http://duke.summon.serialssolutions.com/search?s.fvf[]=ContentType%2CModel%2Cf&s.fvf[]=ContentType%2CReport%2Cf&s.fvf[]=ContentType%2CDatabase%2Cf&s.fvf[]=ContentType%2CArtifact%2Cf&s.fvf[]=ContentType%2CComputer+File%2Cf&s.fvf[]=ContentType%2CPatent%2Cf&s.fvf[]=ContentType%2CTechnical+Report%2Cf&s.fvf[]=ContentType%2CStandard%2Cf&s.fvf[]=ContentType%2CBook+Review%2Ct&s.fvf[]=ContentType%2CNewspaper+Article%2Ct&s.fvf[]=ContentType%2CReference%2Cf&s.fvf[]=ContentType%2CPresentation%2Cf&s.fvf[]=ContentType%2CData+Set%2Cf&s.fvf[]=ContentType%2CArchitectural+Drawing%2Cf&s.fvf[]=ContentType%2CRealia%2Cf&s.fvf[]=ContentType%2CAtlas%2Cf&s.fvf[]=ContentType%2CArchival+Material%2Cf&s.light=t&s.cmd=addFacetValueFilters%28Library,Duke+Internet+Resource,Perkins%2FBostock+Library,Ford+Library,Perkins+Public+Documents%2FMaps,Music+Library,Perkins%2FBostock+Library+%7BV%7D,Lilly+Library,Marine+Lab+Library,Library+Service+Center,Goodson+Law+Library,Rubenstein+Library,Medical+Center+Library,Biol-Env.+Sciences+Library,Divinity+School+Library,University+Archives%29&s.q=' . $queryTerms . '" class="callbox" style="margin-left: 10px;" onclick="_gaq.push([\'_trackEvent\', \'BentoResults\', \'Articles\', \'SeeAll\']);">See&nbsp;All&nbsp;&raquo;</a></h2>
			...
			for ($i = 0; $i < count($theData['documents']); $i++) {
				...
				echo '<div class="document-frame">';
					...
					echo '<div class="title">';
						echo '<div class="text">';
							echo '<h3 class="resultTitle"><a href="' . $document["link"] . '" onclick="_gaq.push([\'_trackEvent\', \'BentoResults\', \'OtherResources\', \'ItemTitle' . $resultCount . '\']);">' . $document["Title"][0] . '</a></h3>';
						echo '</div>';
					echo '</div>';
					...
					echo '<div class="document-summary">';
						...
						if($authorList != "") {
							...
							echo 'by <a href="http://duke.summon.serialssolutions.com/search?s.dym=false&s.q=Author%3A%22' . str_replace(',', '%2C+', $authorList) . '%22" onclick="_gaq.push([\'_trackEvent\', \'BentoResults\', \'OtherResources\', \'ItemAuthor' . $resultCount . '\']);">' . $authorListDisplay . '</a>';
						}
						...
						echo '<div class="content-type">';
							...
							if($document["hasFullText"] == 1) {
								...
								echo ': <a href="' . $document["link"] . '" onclick="_gaq.push([\'_trackEvent\', \'BentoResults\', \'OtherResources\', \'ItemFulltext' . $resultCount . '\']);">Full Text Online</a>';
