...
$urlString = "https://search.library.duke.edu/search?Nao=0&Nr=200092&N=206474&affiliation=DUKE&Ntk=Keyword&output-format=xml&Ntt=";
...
$endecaXMLArchivalStart = microtime(true);
...
$endecaXMLArchivalEnd = microtime(true);
...
if ($theSearch != "" && $searchResults != "-1") {
	...
	// there is a node!
		...
		$internalTotalResults = (int)$numTotalResults;
...
?>
	...
	<div class="results-block first" id="results-archival-materials">
		...
		<div class="resultsHeader">
				...
				<h3><div class="anchor-highlight hide">»</div> Archival Materials <a href="//search.library.duke.edu/search?Nao=0&Nr=200092&N=206474&affiliation=DUKE&Ntk=Keyword&Ntt=<?php echo $theSearch; ?>" class="callbox" style="margin-left: 10px;" <?php echo 'onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'ArchivalMaterials\', eventLabel: \'SeeAll\'});"' ?>>See&nbsp;All&nbsp;&raquo;</a></h3>
		...
		if ($searchResults != "0" AND $searchResults != "-1" AND $theSearch != "") {
			...
			while ($i < $maxResults):
				...
				// sniff out collection guides
				$FindingAid = $theXML->xpath('/trln-endeca-results/results-data/endeca-records-list/records/item['.$i.']/properties/Finding-Aid-URL/item');

				if (isset($FindingAid[0])) {

					if (stripos($FindingAid[0], 'collection guide') !== false) {

							//echo 'found one!';
							//echo 'findingaid: ' . $FindingAid[0];

							// skip this item
							$i++;
							$maxResults++; // need to increase max to account for skipped items
							$internalTotalResults--; // keeping track so we can break if it's the only item
							unset($FindingAid);
							continue;
					}
							...
							echo '<div class="document-frame" titlelocalid="' . $theID . '">';
								...
								echo '<div class="title">';
									echo '<div class="text">';
										...
										echo '<h3 class="resultTitle"><a href="//search.library.duke.edu/search?id=' . $itemPrepend . $theID . '" onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'ArchivalMaterials\', eventLabel: \'ItemTitle' . $resultCount . '\'});">' . $theTitle . '</a></h3>';
								...
								echo '<div class="document-summary">';
									...
									if ($theAuthor != "NONE") {
										echo 'by <a href="//duke.summon.serialssolutions.com/search?s.dym=false&s.q=Author%3A%22' . $theAuthor . '%22" onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'ArchivalMaterials\', eventLabel: \'ItemAuthor' . $resultCount . '\'});">' . $theAuthor . '</a>';
									...
									echo '<div class="holdings-wrapper">';
										...
										if ($holdingsCount) {
											...
											if ($holdingsCount == 1) {
												...
												echo '<div class="more-holdings">There is ' . $holdingsCount . ' additional item available &ndash; <a href="//search.library.duke.edu/search?id=' . $itemPrepend . $theID . '" onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'Archival Materials\', eventLabel: \'ItemMoreHoldings' . $resultCount . '\'});">show more&nbsp;&raquo;</a></div>';

											// multiple holdings
											} else {

												echo '<div class="more-holdings">There are ' . $holdingsCount . ' additional items available &ndash; <a href="//search.library.duke.edu/search?id=' . $itemPrepend . $theID . '" onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'ArchivalMaterials\', eventLabel: \'ItemMoreHoldings' . $resultCount . '\'});">show more&nbsp;&raquo;</a></div>';
		...
		if($searchResults != "0" AND $searchResults != "-1" AND $theSearch != "" AND $internalTotalResults > 1 AND $maxResults < 24) {
			...
			echo '<div class="see-all">';
				...
				echo '<a href="//search.library.duke.edu/search?Nao=0&Nr=200092&N=206474&affiliation=DUKE&Ntk=Keyword&Ntt=' . $theSearch . '" onClick="ga(\'send\', \'event\', { eventCategory: \'BentoResults\', eventAction: \'ArchivalMaterials\', eventLabel: \'SeeAllBottom\'});">See all <strong>' . number_format($numTotalResults) .'</strong> archival material results at Duke</a>';
...
$endecaArchivalEnd = microtime(true);
...
if ($bentoLogging == 1) {
	...
	global $endecaArchivalCreationTime;
	global $endecaXMLArchivalCreationTime;

	$endecaArchivalCreationTime = ($endecaArchivalEnd - $endecaArchivalStart);
	$endecaXMLArchivalCreationTime = ($endecaXMLArchivalEnd - $endecaXMLArchivalStart);
